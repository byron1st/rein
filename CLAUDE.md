...
- Wrap a stdlib/third-party error once with a per-call-site sentinel that names the failure, e.g. `errors.Join(ErrFailedToReadFile, err)`; pass through errors that already carry a sentinel.
